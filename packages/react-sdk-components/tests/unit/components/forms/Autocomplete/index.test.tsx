...
const getDataPage = jest.fn(() =>
  Promise.resolve([
    { pyGUID: '1', name: 'Option 1' },
    { pyGUID: '2', name: 'Option 2' }
  ])
);
...
jest.mock('../../../../../src/components/helpers/utils', () => ({
  ...
  default: {
    getOptionList: () => [
      { key: '1', value: 'Associated Option 1' },
      { key: '2', value: 'Associated Option 2' }
    ]
  }
}));
...
//     if (name === 'TextInput') {
//       return props => <input data-testid='text-input' value={props.value} readOnly />;
//     }
//     if (name === 'FieldValueList') {
//       return ({ name, value, variant }) => <div data-testid={`field-value-${variant ?? 'default'}`}>{`${name}: ${value}`}</div>;
//     }
//     return null;
//   }
// }));
...
const getPConnect = jest.fn(
  () =>
    ({
      getContextName: () => 'appContext',
      getActionsApi: () => ({}),
      getStateProps: () => ({ value: '.autoComplete' }),
      getDataObject: () => ({})
    }) as any
);
...
const getDefaultProps = () => ({
  ...
  listType: 'associated',
  parameters: {},
  datasource: [{ key: '1', value: 'Associated Option 1' }],
  columns: [
    { key: 'true', value: 'key' },
    { display: 'true', primary: 'true', value: 'value' }
  ],
  onChange: jest.fn(),
  disabled: false
});
...
describe('AutoComplete Component', () => {
  ...
  test('renders with label and helper text', () => {
    ...
    expect(screen.getByText('AutoComplete helper text')).toBeInTheDocument();
    expect(screen.getByLabelText('AutoComplete Label')).toBeVisible();
  });
  ...
  test('renders with validation message overriding helper text', () => {
    ...
    expect(screen.queryByText('AutoComplete helper text')).not.toBeInTheDocument();
  });

  test('calls handleEvent on selection change', () => {
    const props = getDefaultProps();
    render(<AutoComplete {...props} />);
    const input = screen.getByLabelText('AutoComplete Label');
    fireEvent.change(input, { target: { value: 'Associated Option 1' } });
    fireEvent.blur(input);
    expect(handleEvent).toHaveBeenCalledWith(expect.any(Object), 'changeNblur', '.autoComplete', '');
  });
  ...
  test('renders in DISPLAY_ONLY mode', () => {
    ...
    props.displayMode = 'DISPLAY_ONLY';
    render(<AutoComplete {...props} />);
    expect(screen.getByTestId('field-value-default')).toHaveTextContent('AutoComplete Label: 1');
  });
  ...
  test('renders in STACKED_LARGE_VAL mode', () => {
    ...
    props.displayMode = 'STACKED_LARGE_VAL';
    render(<AutoComplete {...props} />);
    expect(screen.getByTestId('field-value-stacked')).toHaveTextContent('AutoComplete Label: 1');
  });

  test('renders in readOnly mode with mapped value', () => {
    const props = getDefaultProps();
    props.readOnly = true;
    render(<AutoComplete {...props} />);
    expect(screen.getByTestId('text-input')).toHaveAttribute('value', 'Associated Option 1');
  });
  ...
  test('loads options when listType is "associated"', async () => {
    const props = getDefaultProps();
    props.listType = 'associated';
    render(<AutoComplete {...props} />);
    await waitFor(() => {
      expect(screen.getByLabelText('AutoComplete Label')).toBeInTheDocument();
    });
    expect(screen.getByLabelText('AutoComplete Label')).toHaveValue('Associated Option 1');
  });

  // test('loads options when listType is not "associated"', async () => {
  //   const props = getDefaultProps();
  //   props.listType = 'datapage';
  //   props.datasource = 'D_TestPage';
  //   props.parameters = {};
  //   props.columns = [
  ...
  //   render(<AutoComplete {...props} />);
  //   await waitFor(() => {
  //     expect(getDataPage).toHaveBeenCalledWith('D_TestPage', {}, 'appContext');
  //   });
  //   expect(screen.getByLabelText('AutoComplete Label')).toBeInTheDocument();
  // });
});
